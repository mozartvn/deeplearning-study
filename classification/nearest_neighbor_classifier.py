...
class NearestNeighbor(object):
  ...
  def predict(self, X):
    ...
    for i in xrange(num_test):
      ...
      # using the L1 distance ( sum of asolute value differences )
      distance = np.sum(np.abs(self.Xtr - X[i, :], axis = 1))
      min_index = np.argmin(distance) # get the index with smallest distance
      Ypred[i] = self.ytr[min_index] # predic the label of the nearest examples

    return Ypred


Xtr, Ytr, Xte, Yte = load_CIFA10('../data/cifar10/') # a magic function we provide
